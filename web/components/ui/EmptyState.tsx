...
export function EmptyState({ title, description, action, icon }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center gap-3 rounded-card border border-dashed border-control-border px-6 py-12 text-center">
      {icon ? (
        <span aria-hidden="true" className="text-2xl text-ink-muted">
          {icon}
        </span>
      ) : null}
